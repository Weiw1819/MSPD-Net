...
from utils.losses import DenseEnergyLoss, get_loss
...
# from network.model import MAPD
...
parser.add_argument("--config",
                    default='configs/voc_config.yaml',
                    type=str,
                    help="config")
...
def train(cfg):
    ...
    MSPD = MSPD(backbone=cfg.backbone.config,
    ...
    logging.info('\nNetwork config: \n%s' % (MSPD))
    param_groups = MSPD.get_param_groups()
    ...
    logging.info('\nOptimizer: \n%s' % optimizer)
    MSPD = DistributedDataParallel(MSPD, device_ids=[args.local_rank], broadcast_buffers=False, find_unused_parameters=True)
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        cls, segs, attns, attn_pred = MSPD(inputs, seg_detach=args.seg_detach)

        cams, ref_mat = multi_scale_cam_with_ref_mat(MSPD, inputs=inputs, scales=cfg.cam.scales)
        ...
        cls2, segs2, attns2, attn_pred2 = MSPD(img2, seg_detach=args.seg_detach)
        cams2 = multi_scale_cam(MSPD, inputs=img2, scales=cfg.cam.scales)
        ...
        loss_fea = torch.mean(torch.abs(segs1[:, 1:, :, :] - segs2[:, 1:, :, :]))
        loss =  100*feat_feat_mi_estimation(segs1[:, 1:, :, :], segs2[:, 1:, :, :],14) + loss_fea
        ...
        AP_loss, pos_count, neg_count = get_loss(attn_pred, ref_label)
        ...
        AP_loss = AP_loss - 100 * (lossmil - lossmi) - 100 * (lossmil2 - lossmi2)

        cls_loss = F.multilabel_soft_margin_loss(cls, cls_labels)

        if n_iter <= cfg.train.cam_iters:
            loss = 1.0 * cls_loss  + 0.0 * loss + 0.0 * (AP_loss)
        else:
            loss = 1.0 * cls_loss  + 0.1 * (
            AP_loss) + 0.1 * loss + 0.1 * CIML_loss

        avg_meter.add({'cls_loss': cls_loss.item(), 'cam_loss': CIML_loss.item(), 'AP_loss': AP_loss.item(),
                       'corr_loss': loss.item()})
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if (n_iter + 1) % cfg.train.log_iters == 0:
            ...
            if args.local_rank == 0:
                logging.info(
                    "Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; cls_loss: %.4f, AP_loss: %.4f, corr_loss: %.4f, cam_loss %.4f, pseudo_seg_mAcc: %.4f" % (
                        n_iter + 1, delta, eta, cur_lr, avg_meter.pop('cls_loss'), avg_meter.pop('AP_loss'),
                        avg_meter.pop('corr_loss'), avg_meter.pop('cam_loss'), seg_mAcc))

        if (n_iter + 1) % cfg.train.eval_iters == 0:
            ckpt_name = os.path.join(cfg.work_dir.ckpt_dir, "MSPD_iter_%d.pth" % (n_iter + 1))
            if args.local_rank == 0:
                logging.info('Validating...')
                torch.save(MSPD.state_dict(), ckpt_name)
            cls_score, cam_score, ref_score = validate(model=MSPD, data_loader=val_loader, cfg=cfg)
            if args.local_rank == 0:
